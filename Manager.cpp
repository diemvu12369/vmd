...
Utility *bg, *coin, *mnBg;
Button *yasuoButton, *pinkButton, *nikoButton, *playButton;

Utility *coinsNum, *waveText, *waveNum, *gameOver;
...
std::vector<Action *> allies, enemies;
...
void initYasuo(Action *&tmp, bool side) {
    tmp = new Action();
...
void initPinkSkull(Action *&tmp, bool side) {
    tmp = new Action();
...
void initGreenSkull(Action *&tmp, bool side) {
    tmp = new Action();
...
void initNiko(Action *&tmp, bool side) {
    tmp = new Action();
...
void Manager::init(const char *title, int x_pos, int y_pos, int width, int height, bool fullscreen) {
    ...
    coinsNum = new Utility();
    waveText = new Utility();
    waveNum = new Utility();
    gameOver = new Utility();
    ...
    std::cerr << "Y\n";
    waveText->setObjTexture(TextureDrawer::loadText("Wave", waveText->destR));
    std::cerr << "Z\n";
...
void Manager::handleEvents() {
    ...
    if (!killed) {
        if (wave + 1 < (int) wavesMonster.size()) {
            score += wallet * 2;
            wave++;
            for (int i = 0; i < (int)wavesMonster[wave].size(); ++i) {
    ...
    yasuoButton->setActive(wallet >= 13 && currentTime >= yasuoButton->timeSinceLastOrder + 1000);
    pinkButton->setActive(wallet >= 30 && currentTime >= pinkButton->timeSinceLastOrder + 1000);
    nikoButton->setActive(wallet >= 120 && currentTime >= nikoButton->timeSinceLastOrder + 1000);
}
...
void Manager::update() {
    if (is_over) {
        ...
        for (auto ally:allies)
            ally->Update();
        coinsNum->setObjTexture(TextureDrawer::loadNumber(wallet, coinsNum->destR, 30));
        waveNum->setObjTexture(TextureDrawer::loadNumber(wave + 1, waveNum->destR, 30));
    }
}
